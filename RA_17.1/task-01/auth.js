const express = require("express");
const fs = require("fs");
const jwt = require("jsonwebtoken");
const router = express.Router();

const users = JSON.parse(fs.readFileSync("users.json", "utf8"));
const SECRET_KEY =
  "";

router.post("/", (req, res) => {
  const { email, password } = req.body;
  const user = users.find((u) => u.email === email && u.password === password);

  if (user) {
    const userData = { id: user.id, role: user.role };
    // TODO: Replace the hardcoded token with a JWT token generated using the `userData` object.
    // Use the jwt.sign() method to create the token with an expiration time of 1 hour.
    // The SECRET_KEY should be used to sign the token securely.
    const token = "123";
    res.json({ token });
  } else {
    res.status(401).send("Authentication failed");
  }
});

module.exports = router;
