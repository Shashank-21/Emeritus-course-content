...
function findCityWithHighestAverage(analyzedData) {
  let highestAvg = analyzedData[0].avgTemp;
  let cityName = "";
  for (let item of analyzedData) {
    if (item.avgTemp > highestAvg) {
      highestAvg = item.avgTemp;
      cityName = item.city;
    }
  }
  return cityName;
...
function findCityWithLowestAverage(analyzedData) {
  let lowestAvg = analyzedData[0].avgTemp;
  let cityName = "";
  for (let item of analyzedData) {
    if (item.avgTemp < lowestAvg) {
      lowestAvg = item.avgTemp;
      cityName = item.city;
    }
  }
  return cityName;
